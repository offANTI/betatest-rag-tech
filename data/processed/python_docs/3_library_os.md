...
SEEK_SETAdjust the file position relative to the beginning of the file.

SEEK_CURAdjust the file position relative to the current file position.

SEEK_ENDAdjust the file position relative to the end of the file.
...
SEEK_DATAAdjust the file offset to the next location containing data,
relative to the seek position.

SEEK_HOLEAdjust the file offset to the next location containing a hole,
relative to the seek position.
...
See also
The contextlib.chdir() context manager, which changes the current
...
original scandir() argument.  If the scandir() path
...
not for joining with the entry name.  If the scandir() path
...
Windows MAX_PATH and those described in Linux

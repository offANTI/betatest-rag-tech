...
raise a TypeError. If content_manager is not specified, use
...
Originally added in 3.4 as a provisional module.  Docs for legacy message class moved to
email.message.Message: Representing an email message using the compat32 API.

The EmailMessage class requires a policy that provides a
content_manager attribute for content management methods like
set_content() and get_content() to work. The legacy
compat32 policy does not support these methods
and should not be used with EmailMessage.
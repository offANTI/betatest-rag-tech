...
Event loop: Each thread should have its own event loop which
...
Task management: Tasks and futures created in one thread should
not be awaited or manipulated from another thread.

Thread-safe APIs: When interacting with asyncio from multiple
...
Synchronization: The synchronization primitives provided by

...
When subclassing Enum, mix-in types must appear before the
Enum class itself in the sequence of bases, as in the IntEnum
example above.

Mix-in types must be subclassable. For example, bool and
...
A data type is a mixin that defines __new__(), or a
dataclass

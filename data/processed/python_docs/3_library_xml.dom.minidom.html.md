...
entities and
notations are read-only and support only
len() and subscription by a name.

strictErrorChecking and
specified are always False.

removeAttribute() and
removeAttributeNS() raise
NotFoundErr if there is no matching attribute,

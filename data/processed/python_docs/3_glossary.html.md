...
A callable that can be called to retrieve the annotations of
an object. Annotate functions are usually functions,
automatically generated as the __annotate__ attribute of functions,
...
can be retrieved by calling annotationlib.get_annotations()
...
function or an asynchronous generator iterator, depending on
...
except that it contains yield expressions for producing a
series of values usable in an async for loop.  See PEP 525.
...
free-threaded builds of Python, threads can
...
identifier as defined by Names (identifiers and keywords), for example using
...
free variables, but only x is a closure variable:
def outer():
    x = 0
    def inner():
        nonlocal x
...
race conditions, and might also trigger a
...
contextvars.Context object and accessed via
...
controls the environment seen in a with statement.  See
PEP 343.
...
exposed in a Python API.  See also race condition and
thread-safe.
...
multiple locks in a consistent order.  See also
lock and reentrant.
...
special method. For example, __init__ is often pronounced
“dunder init”.
...
of an object, such as the value of type aliases created with the type
statement.
...
See Yield expressions.


#### generator iterator

An object created by a generator function or a
...
Generator iterators also implement the send() method
to send a value into the suspended generator, and the
throw() method to raise an exception at the point
where the generator was paused.  See Generator-iterator methods.
...
run with -X gil=0 or after setting the PYTHON_GIL=0
...
in a dictionary.  Immutable objects are inherently thread-safe
...
define __iter__().
And also please note that free-threaded
CPython does not guarantee thread-safe behavior of iterator
operations.
...
This issue can be solved with locks or by using the
EAFP approach.  See also thread-safe.
...
elements is O(1).  See Time complexity of operations on built-in types.
...
will block until the lock becomes available.  Python’s threading
module provides Lock (a basic lock) and
RLock (a reentrant lock).  Locks are used
to prevent race conditions and ensure
thread-safe access to shared data.  Alternative design patterns
...
operations that hold locks. In free-threaded
Python, built-in types like dict and list provide
...
race conditions.  See also immutable,
thread-safe, and concurrent modification.
...
C, C++, Rust or Fortran code in extension modules
...
Proper synchronization using locks and other
synchronization primitives helps
...
global interpreter lock (GIL), only one
...
a global lock shared across all objects. In free-threaded Python, built-in types like dict and
list use per-object locks to allow concurrent operations on
...
on the same object from proceeding, but do not block lock-free
operations.
...
non-deterministic behavior and bugs that are difficult to
reproduce.  A data race is a specific type of race condition
involving unsynchronized access to shared memory.  The LBYL
coding style is particularly susceptible to race conditions in
multi-threaded code.  Using locks and other
...
functions may be non-deterministic if they’re called reentrantly in a
...
The collection of packages, modules
and extension modules distributed as a part
...
subscription expression, for example,
the 3 in items[3].
...
including Lock, RLock,
Semaphore, Condition,
Event, and Barrier.  Additionally,
...
locks to protect shared mutable state, or is designed
to avoid shared mutable state entirely.  In the
free-threaded build, built-in types like
dict, list, and set use internal locking
...
The tokenize module exposes Python’s lexical analyzer.

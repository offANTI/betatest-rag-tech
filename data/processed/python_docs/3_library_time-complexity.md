...
We use Big O notation to describe how the running time of an operation grows
...
implementation, see How are dictionaries implemented in CPython?.
...
Difference (s1 - s2, s1.difference(s2)) [7] [9]

O(len(s1))

Difference update (s1 -= s2, s1.difference_update(s2)) [1] [7] [8]

O(min(len(s1), len(s2)))

Symmetric difference (s1 ^ s2, s1.symmetric_difference(s2)) [7]

O(len(s1) + len(s2))

Symmetric difference update (s1 ^= s2, s1.symmetric_difference_update(s2)) [1] [7]
...
Substring search (x in s, s.find(x), s.index(x)) [11]

O(n)

Reverse substring search (s.rfind(x), s.rindex(x)) [11] [12]
...
Index and count (r.index(x), r.count(x)) [15]
...
Amortized. An individual operation may occasionally be O(n)
...
Popping or deleting the element at index k of a list of size n
...
Plus the cost of iterating over t, which may be expensive for an
arbitrary iterable.
...
See Objects/listsort.txt for more information.

The number of elements is stored in the object, so len() does
not need to count them.

Copying a frozenset is O(1) as it
returns the original object.

These operations scan the container’s internal hash table, which is
...
O(len(t)) if t is not a set.

O(len(s) + len(t)) if t is not a set.

Each concatenation builds a new object, so building a string by
concatenating many pieces in a loop is quadratic in the total length.
See the note on concatenating immutable sequences for alternatives.

With start and end arguments, n is the length of the region
searched rather than of s, and unlike slicing nothing is copied.

This is the worst case. Reverse searches are O(n) on typical
...
This assumes a codec that does a constant amount of work per character.

These unpack and compare each element individually, so they are much

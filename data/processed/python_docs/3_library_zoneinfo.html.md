...
The default TZPATH when not otherwise specified can be configured at
compile time.

TZPATH can be configured using an environment variable.

At runtime, the search path can be
...
ZoneInfo.from_file(file_obj, /, key=None): When constructed from a file, the

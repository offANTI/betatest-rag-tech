...
or python -O -c 'assert await 1' now produce SyntaxErrors.
...
dump_c_stack() function or via the c_stack argument
...
Signature.format() takes a new argument unquote_annotations.
...
on a mutually exclusive group now both raise ValueErrors.
...
The check_home argument of sysconfig.is_python_build() has been
...
When using the functional syntax of TypedDicts, failing
...
pickle.dump() and pickle.dumps() now raise
PicklingError for some failures that previously raised
AttributeError, ImportError, ValueError,
UnicodeEncodeError or PicklingError,

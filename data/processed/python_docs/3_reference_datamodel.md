...
A reference to the dictionary that holds the function’s
global variables – the global namespace of the module
...
None or a tuple of cells that contain bindings for the names specified
in the co_freevars attribute of the function’s
code object.
...
See also: __qualname__ attributes.
...
A tuple containing default parameter values
...
if the function has no annotations. See object.__annotate__.
...
A tuple containing the type parameters of
a generic function.
...
A string if the original function had a docstring, else
None.
...
not have a __path__ attribute. See __path__ attributes on modules for
more details.
It is strongly recommended that you use
...
For best practices on working with __annotations__,
please see annotationlib. Use
annotationlib.get_annotations() instead of accessing this
attribute directly.
...
The annotate function for this class, or None
if the class has no annotations.
See also: __annotate__ attributes.
...
Setting the __module__ attribute removes the
...
The tuple of classes that are considered when looking for
...
section on __slots__ for more details.
...
The __hash__(), __iter__(),
__reversed__(), __contains__(),
__class_getitem__() and __fspath__()
...
(such as __add__()) fails then the operation is not supported, which is why the
